...
namespace PureIP
{
    class Program
    {
        ...
        public Program(string ipfilePath)
        {
        ...
        {
            string Readip;
            string ipfilePath = @".\QQWry.dat";
            Program ipSearch = new Program(ipfilePath);

            Program.IPLocation loc;
            try
            {
                Readip = args[0];
                loc = ipSearch.GetIPLocation(Readip);
            }
            catch
            {
                Console.WriteLine("参数传递有误");
                return;
            }
            Console.WriteLine("你查的ip是：{0} 地理位置：{1} {2}", Readip, loc.country, loc.area);
        }///<summary>
         /// 地理位置,包括国家和地区
         ///</summary>
